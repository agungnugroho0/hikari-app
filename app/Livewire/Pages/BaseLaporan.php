...
abstract class BaseLaporan extends Component
{
    ...
    public bool $lockClassSelection = false;
    ...
    public function mount(): void
    {
        ...
        $this->selectedClassId = $this->resolveInitialClassId();
        $this->normalizeSelectedClass();
    }
    ...
    {
        return data_get($this->classOptions->first(), 'id');
    }
    ...
    protected function restrictedWithoutClass(): bool
    {
        return $this->lockClassSelection && $this->classOptions->isEmpty();
    }

    protected function emptyChartSeries(): array
    {
        return [
            'series' => $this->buildMonthlySeries(collect()),
            'year_total' => 0,
            'max_total' => 1,
            'line_points' => $this->buildLinePoints($this->buildMonthlySeries(collect())),
            'area_points' => $this->buildAreaPoints($this->buildMonthlySeries(collect())),
        ];
    }

    protected function normalizeSelectedClass(): void
    {
        $validClassIds = $this->classOptions->pluck('id');
        ...
        if (! $validClassIds->contains($this->selectedClassId)) {
            $this->selectedClassId = $validClassIds->first();
        }
    }
    ...
    {
        $classIds = $this->eligibleClassQuery()->pluck('id_kelas');

        if ($this->lockClassSelection && $classIds->isEmpty()) {
            return collect([(int) now()->format('Y')]);
        }

        $graduationYears = ListLolos::query()
            ->join('core', 'core.nis', '=', 'list_lolos.nis')
            ->when($classIds->isNotEmpty(), fn ($query) => $query->whereIn('core.id_kelas', $classIds))
    ...
    {
        return $this->eligibleClassQuery()
            ->orderBy('tingkat')
            ->orderBy('nama_kelas')
    ...
    public function totalGraduationChart(): array
    {
        if ($this->restrictedWithoutClass()) {
            return $this->emptyChartSeries();
        }

        $classIds = $this->classOptions->pluck('id');

        $rows = DB::table('list_lolos')
            ->join('core', 'core.nis', '=', 'list_lolos.nis')
            ->selectRaw('MONTH(tgl_lolos) as month_number, COUNT(DISTINCT list_lolos.nis) as total')
    ...
    {
        if ($this->restrictedWithoutClass()) {
            $series = $this->buildDailySeries(Carbon::create($this->year, $this->month, 1)->startOfMonth(), collect());
    ...
    public function attendanceChart(): array
    {
        ...
        $totalRecordedAttendances = max(1, (int) $rows->sum('total'));
        ...
        $recap = collect(self::ATTENDANCE_STATUSES)
            ->map(function (array $status, string $key) use ($rows, $totalRecordedAttendances) {
                ...
                return [
                    ...
                    'background' => $status['background'],
                    'total' => $total,
                    'percentage' => round(($total / $totalRecordedAttendances) * 100, 2),
                ];
            })
            ->values()
            ->all();
